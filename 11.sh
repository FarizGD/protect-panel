...
echo "🚀 Memasang proteksi Anti Setting Nodes..."
...
class NodesController extends Controller
{
    ...
    public function store(NodeFormRequest $request): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "🚫 You do not have permission to add nodes. Only admin ID 1 can! - Protect by FarizGD (ngapain lu anj)");
        }
    ...
    public function updateSettings(NodeFormRequest $request, Node $node): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "⚠️ Access denied only admin ID 1 can edit nodes. - Protect");
        }
    ...
    public function delete(int|Node $node): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "❌ Access denied only ID 1 can delete node. - Protect");
        }
...
echo "✅ Proteksi Anti Setting Nodes berhasil dipasang!"
echo "📂 Lokasi file: $REMOTE_PATH"
echo "🗂️ Backup file lama: $BACKUP_PATH (jika sebelumnya ada)"
echo "🔒 Hanya Admin (ID 1) yang bisa Setting Nodes."
